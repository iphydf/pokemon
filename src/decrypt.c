...
static struct Cipher8 CreateCipherIV(struct IV iv)
{
  struct Cipher8 cipher8;
  for (int i = 0; i < 8; i++)
  {
    for (int j = 0; j < 32; j++)
    {
...
void decrypt(const unsigned char* input_ciphertext, const size_t input_size, unsigned char* output_plaintext, size_t* output_size)
{
  // the rounded size must be %% 256 == 32
  //const size_t rounded_input_size = input_size + (256 - (input_size % 256)) + 32;
  //const size_t rounded_size = 
  // payload_size must be divisble by 256
  size_t rounded_input_size;
  if (input_size % 256 != 32)
  {
    rounded_input_size = input_size + (256 - (input_size % 256)) + 32;
  }
  else
  {
    rounded_input_size = input_size;
  }
  size_t payload_size = rounded_input_size - 32;

  // Allocate space for decrypted payload
  uint8_t* output = malloc(payload_size);
  memcpy(output, input_ciphertext + 32, payload_size);
